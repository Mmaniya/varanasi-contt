...
if($searchValue != ''){
    ...
    foreach ($result as $key =>$value){
        ...
        $Cselected = '';
        $Mselected = '';
        if($value->religion == 'Hindu'){
            $Hselected = 'selected';
        }else if($value->religion == 'Christian'){
            $Cselected = 'selected';
        }else if($value->religion == 'Muslim'){
            $Mselected = 'selected';
        }
        $action ='<select name="religion" id="'.$value->id .'" onchange="updatereligion(this.value,this.id)">
                    <option value="Hindu" '.$Hselected.'>Hindu</option>
                    <option value="Christian" '.$Cselected.'>Christian</option>
                    <option value="Muslim" '.$Mselected.'>Muslim</option>
                </select>';
