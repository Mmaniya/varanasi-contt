                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody id="draggableEmployee" class="draggable">
                            ...
                            <tr class="row_id_<?php echo $value->id; ?>" id="<?php echo $value->id; ?>">
                                ...
                                <td><a href="javascript:void(0);" onclick="view_category(<?php echo $value->id; ?>)"  class="text-primary"><?php echo $value->first_name.'&nbsp;'.$value->last_name ?></a></td>
                                <td>
...
<?php if($action == 'employee_consultancy_table'){ ?>
    <div class="card-header bg-c-lite-green;">
        <h5>Consultancy List</h5>
        <a href="javascript:void(0);" style="font-size:16px;" onclick="add_edit_consultancy('')"
            class="right-float label label-success"> <i class="feather icon-plus"> Add New</i></a>
    </div>
    <div class="card-block">
        <div class="card-block table-border-style">
                ...
                <form action="javascript:void(0);" id="employee_role_position" style="width:100%">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody id="draggableRole" class="draggable">
                            ...
                            $employeeObj->status = $_POST['status'];   
                            $rsConsultancy = $employeeObj->get_emp_consultancy();
                            if (count($rsConsultancy) > 0) {
                                foreach ($rsConsultancy as $key => $value) { ?>

                            <tr class="row_id_<?php echo $value->id; ?>" id="<?php echo $value->id; ?>">
                                <input type="hidden" name="consultancy_id[]" value="<?php echo $value->id ?>">
                                <th><?php echo $key + 1 ?></th>
                                <td><a href="javascript:void(0);" onclick="view_category(<?php echo $value->id; ?>)"  class="text-primary"><?php echo $value->name ?></a></td>
                                <td>
                                    <a href="javascript:void(0);" class="label label-info" onclick="add_edit_consultancy(<?php echo $value->id; ?>)"><i class="fa fa-edit"  aria-hidden="true"></i>Edit</a>
                                    <a href="javascript:void(0);" class="label label-danger" onclick="delete_employee_consultancy(<?php echo $value->id; ?>)"><i class="fa fa-trash" aria-hidden="true"></i>Delete</a>                                  
                                </td>
                                <td>
                                    <label class="switch">
                                        <input type="checkbox" class="status_update_<?php echo $value->id; ?>" onchange="statusConsultancy(<?php echo $value->id; ?>)" <?php echo $statusArr[$value->status]; ?>>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                            </tr>
                            <?php }} else {?>
                            <tr>
                                <td colspan="4" class="text-center"> No Records Found. Click here to <a href="javascript:void(0);" onclick="add_edit_consultancy('')" style="color:#01a9ac"> Add New</a> </td>
                            </tr>
                            <?php }?>
                        </tbody>
                    </table>
