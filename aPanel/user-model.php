               ...
               <div class="boothList_div">
                  <ul style="line-height: 31px;">
                     <?php 
                        ...
                        echo '<li id="list_'.$val->id.'" calss="remove'.$val->id.'"> '.$val->booth_no.' - '.$val->booth_name.'  <input type="hidden" name="booth_id[]" value="'.$val->id.'"/> <a href="javascript:void(0)" style="color:red;" onclick="removeBooth('.$val->id.','.$user_id.')">Remove</a></li>';
                        }
                        }		 
                                ?>
                  </ul>
...
$( document ).ready(function() {
   ...
   function removeBooth(booth_id,id){   $('#list_'+booth_id).remove();
